...
os.makedirs('/home/siddharth/liver_cirrhosis/ml/results', exist_ok=True)
...
# CLR transformation with pseudocount to handle zeros
X_pseudo = X_raw + 1e-6
gm = np.exp(np.log(X_pseudo).mean(axis=1))
X = np.log(X_pseudo.div(gm, axis=0))
...
print("\n" + "="*65)
print("  LODO VALIDATION — Leave One Dataset Out (CLR normalized)")
print("="*65)
...
results_df.to_csv('/home/siddharth/liver_cirrhosis/ml/results/lodo_clr_results.csv', index=False)
...
print("\nFull results saved to: ml/results/lodo_clr_results.csv")
print("="*65)
